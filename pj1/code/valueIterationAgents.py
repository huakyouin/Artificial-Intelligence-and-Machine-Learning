...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for iteration in range(self.iterations):
            old_v = self.values.copy() # Remember old values
                ...
                if not self.mdp.isTerminal(s):
                    ...
                    for a in self.mdp.getPossibleActions(s):
                        for s_next,p in self.mdp.getTransitionStatesAndProbs(s, a):
                            ...
                            q[(s,a)] += (p * (self.mdp.getReward(s,a,s_next) + (self.discount * old_v[s_next])))
                arg_max = q.argMax()
                self.values[s] = q[arg_max]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for s_next,p in self.mdp.getTransitionStatesAndProbs(state,action):
            q += (p * (self.mdp.getReward(state,action,s_next) + (self.discount * self.values[s_next])))
        return q

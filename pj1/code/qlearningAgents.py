...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for key in featureVector:
            total += (self.weights[key] * featureVector[key])
        return total
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        next_qs = [self.getQValue(nextState,nextAction) for nextAction in self.getLegalActions(nextState)]
        max_next_q = max(next_qs) if len(next_qs) > 0 else 0
